...
export default function Contact({ onClose }) {
  ...
  useEffect(() => {
    ...
    const updateServicePlacement = () => {
      ...
      const menuHeight =
        serviceMenuRef.current?.getBoundingClientRect().height ?? 260;
      const gap = 4;
  ...
  const handleServiceKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown" || e.key === "ArrowUp") {
      ...
      if (!serviceOpen) {
        setServiceHighlight(
          form.service ? serviceOptions.indexOf(form.service) + 1 : 0,
        );
        setServiceOpen(true);
        return;
      }
      ...
      setServiceHighlight((current) => {
        ...
        return (
          (current + direction + serviceOptions.length + 1) %
    ...
    if ((e.key === "Enter" || e.key === " ") && serviceOpen) {
      e.preventDefault();
      selectService(
        serviceHighlight === 0 ? "" : serviceOptions[serviceHighlight - 1],
      );
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setStatus("error");
        setErrorMessage(
          data.message || "We couldn't send that. Please try again.",
        );
      }
    } catch {
      setStatus("error");
      ...
      >
        ...
        >
          ...
          <p className="mb-4 text-nav/relaxed text-ink-mid">
            Ready to start your project? Fill in the form and our team will get
              ...
              <div ref={serviceRef} className={`${fieldClass} relative`}>
                ...
                <button
                  ...
                  onClick={() => {
                    setServiceHighlight(
                      form.service
                        ? serviceOptions.indexOf(form.service) + 1
                        : 0,
                    );
                ...
                >
                  ...
                  >
                    ...
                    <Check
                      ...
                      aria-hidden="true"
                      className={
                        !form.service || serviceHighlight === 0
                          ? "text-brand"
                          : "invisible"
                      }
                    />
                ...
                {errors.service && (
                  <span className={errorClass}>{errors.service}</span>
                )}
              </div>
