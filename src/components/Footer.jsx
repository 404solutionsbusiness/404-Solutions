...
const linkClass = `inline-block font-medium text-body-xs text-ink-soft no-underline
   transition-[color,transform] duration-200 ease-clay
   hover:translate-x-0.5 hover:text-brand-btn sm:text-desc`;

const headingClass =
  "mt-0.5 mb-4 font-extrabold text-xs text-ink sm:mb-4.75 sm:text-sm";
...
const socialBase = `flex size-9 shrink-0 items-center justify-center rounded-xs border no-underline
      ...
      <div className="mx-auto w-full max-w-page px-5 md:px-8 lg:px-12">
        ...
        >
          ...
          <div className="col-span-full flex min-w-0 flex-col items-start xl:col-span-1">
            <a
              href="#home"
              className="mb-4 inline-flex items-center no-underline sm:mb-4.5"
              aria-label="404 Solutions home"
            >
              <img
                src="/logo.png"
                alt="404 Solutions"
            ...
            <p className="mb-5.25 max-w-72.5 font-medium text-body-xs text-ink-soft sm:mb-6 sm:max-w-82.5 sm:text-xs md:max-w-97.5 md:text-body-sm xl:max-w-70">
              We are a digital solutions company specializing in web
              development, social media and digital growth.
            </p>

            <div
              className="flex items-center gap-2 sm:gap-2.5 md:gap-2.75"
              aria-label="Social media links"
            >
              {socials.map((s, i) => (
                <a
                  key={s.label}
                  href={s.label === "Instagram" ? "https://www.instagram.com/404_solutions_?stkn=MWsxNXphOHU3ZDh1aQ==" : "#"}
                  aria-label={s.label}
                  target={s.label === "Instagram" ? "_blank" : undefined}
                  rel={s.label === "Instagram" ? "noreferrer" : undefined}
          ...
          <div className="flex min-w-0 flex-col items-start md:col-span-1">
            ...
            <ul
              role="list"
              className="flex list-none flex-col gap-2.75 p-0 sm:gap-3.5"
            >
              <li className="flex min-w-0 items-center gap-2 sm:gap-2.25">
                ...
                <a
                  href="mailto:404solutions.business@gmail.com"
                  className={`${linkClass} min-w-0 [overflow-wrap:anywhere]`}
                >
